...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({ success: true, service }, { headers: getCorsHeaders() });
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: 500, headers: getCorsHeaders() });
  }
}
...
export async function PUT(request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "Service ID required" }, { status: 400, headers: getCorsHeaders() });
